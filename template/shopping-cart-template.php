        ...
        <div class="container pb-4 px-4">
            ...
            <div class="row">
                <div class="container rounded bg-white shadow-sm my-4 py-5 px-5">
                    ...
                    <?php endforeach; ?>
                    <?php else: ?><p class="mt-3 lead ml-2">The cart is empty, <a href="api-shop.php">let's explore the shop</a>.</p><?php endif; ?><div class="bg-light rounded-pill my-4 p-4 text-uppercase font-weight-bold text-info"> Total price: <?php echo $templateParams["total_price"]; ?> &euro;</div><?php if($templateParams["logged"]): ?><a href="api-checkout.php" class="btn btn-primary rounded-pill py-2 btn-block">Proceed to checkout</a><?php else: ?><a href="login.php" class="btn btn-primary rounded-pill py-2 btn-block mt-5">Login to go to checkout</a>
                    <?php endif; ?>
                </div>
            </div>
        </div>
